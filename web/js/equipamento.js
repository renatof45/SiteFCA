...
function equipamento(object, tipo) {
    ...
    if (object.name === 'mais_dados') {
            ...
            $.post("index.php/equipamento?get_status_equipamento=0&equipamento=" + object.getAttribute('equipamento_id'), function (data) {
                ...
                if (estado['estado'] === 'Em manutençao' || estado['estado'] === 'Parada - Em Manutenção') {
                    ...
                    $.post("index.php/equipamento?get_etapas=0&status=" + estado.id, function (data) {
                        ...
                        div += '<input type="button" id="historico' + object.getAttribute('equipamento_id') + '" equipamento_id="' + object.getAttribute('equipamento_id') + '"  name="historico" onclick="equipamento(this,' + tipo + ');" style="width:110px;margin-bottom: 5px;float: left;" value="Histórico completo" class="button">';
                        div += '<input type="button" onclick="$(\'#div' + object.getAttribute('equipamento_id') + '\').next().remove();" style="width:110px;margin-bottom: 5px;float: left;" value="Cancelar" class="button"></div>';
                    ...
                    div += '</div>';
                    div += '<input type="button" id="historico' + object.getAttribute('equipamento_id') + '" equipamento_id="' + object.getAttribute('equipamento_id') + '"  name="historico" onclick="equipamento(this,' + tipo + ');" style="margin-left:100px;width:110px;margin-bottom: 5px;float: left;" value="Histórico completo" class="button">';
                    div += '<input type="button" onclick="$(\'#div' + object.getAttribute('equipamento_id') + '\').next().remove();" style="width:110px;margin-bottom: 5px;float: left;" value="Cancelar" class="button"></div>';
                    $("#div" + object.getAttribute('equipamento_id')).after(div + '</div>');
                }
        ...
        if ($("#" + object.id).prev().children().length === 2) {
            $.post("index.php/equipamento?change_satus", {equipamento: object.getAttribute('equipamento_id'), status: $("#halt-status" + object.getAttribute('equipamento_id') + " option:selected").html(), descricao: '', comentario: ''}, function (dat) {
                $('#dvLoading').hide();
        ...
        if ($("#" + object.id).prev().children().length === 5) {
            if ($("#" + object.id).prev().children()[4].children[1].value === ' ') {
                ...
                $.post("index.php/equipamento?change_satus", {equipamento: object.getAttribute('equipamento_id'), status: $("#halt-status" + object.getAttribute('equipamento_id') + " option:selected").html(), descricao: '', comentario: ''}, function (dat) {
                    ...
                    $.post("index.php/pedidos_trabalho?save", {prioridade: prioridade, texto: texto, unidade: unidade, equipamento: equipamento}, function (dat) {
                        $('#dvLoading').hide();
                        $("#div" + object.getAttribute('equipamento_id')).next().remove();
                        $("#div" + object.getAttribute('equipamento_id'))[0].children[3].value = 'Alterar status';
